...
CREATE VIEW v_trajet_le_plus_rentable_avec_info AS SELECT v_trajet_le_plus_rentable.*,nom_trajet FROM v_trajet_le_plus_rentable JOIN cooperative_trajet ON v_trajet_le_plus_rentable.id_trajet=cooperative_trajet.id_trajet;

CREATE OR REPLACE VIEW v_trajet_vehicule AS SELECT htj.id_histo, htj.id_chauffeur, vehicule.id_vehicule, vehicule.nom_vehicule, vehicule.immatriculation FROM cooperative_histo_trajet_jour htj JOIN cooperative_vehicule vehicule ON htj.id_vehicule = vehicule.id_vehicule ;
CREATE OR REPLACE VIEW v_trajet_chauffeur AS SELECT htj.id_histo, chauffeur.id_chauffeur, chauffeur.nom_chauffeur FROM cooperative_histo_trajet_jour htj JOIN cooperative_chauffeur chauffeur ON htj.id_chauffeur = chauffeur.id_chauffeur;
CREATE OR REPLACE VIEW v_histo_trajet_jour AS SELECT htj.id_histo, htj.id_trajet, htj.id_chauffeur, vtv.id_vehicule, vtv.nom_vehicule, vtv.immatriculation, chauffeur.nom_chauffeur,  htj.kw_effectue, htj.montant_recette, htj.montant_carburant, htj.date_trajet_jr, trajet.nom_trajet, trajet.date_debut, trajet.date_fin      FROM cooperative_histo_trajet_jour htj JOIN cooperative_trajet trajet ON htj.id_trajet = trajet.id_trajet JOIN v_trajet_vehicule vtv ON htj.id_vehicule = vtv.id_vehicule JOIN v_trajet_chauffeur chauffeur ON vtv.id_chauffeur = chauffeur.id_chauffeur ;
